...
- **Theme Protocol v2 — live theme updates without restart.** The desktop's `cycleTheme` (`Ctrl+Shift+T`) now broadcasts an OSC 7777 sequence (`ESC ] 7777 ; ziggyzag.theme=<id> BEL`) to every live pane's PTY input. The shell's `handleEscapeSequence` recognises `]` as an OSC introducer and routes to a new `handleOscSequence` reader that consumes bytes until BEL or `ESC \\`, bounded to 256 bytes. The OSC payload is parsed by `applyOscPayload` (extracted helper, four new tests) which updates `Shell.current_theme` in place and clears the prompt snapshot so the next prompt redraws with the new accent. Other OSC codes (window title, hyperlinks, etc.) drain silently. Spec at `docs/reference/theme-protocol.md`.
- **SQLite history backend scaffold** at `apps/shell/src/history_backend.zig` (376 lines). Wave-4 foundation only — not yet wired into the runtime. Defines `schema_sql` (Atuin-shaped DDL with three indices), `HistoryEntry` (7 fields including hostname + session), `HistoryBackend` vtable interface with `init / deinit / append / query / import_tsv / export_tsv`, `QuerySpec` (cwd_prefix, exit_status, since_ms, command_substring, limit), and stub `TsvBackend` + `SqliteBackend` returning `error.NotImplemented` with implementation-plan comments. Four tests confirm the scaffold's shape. Tests are wired into the existing shell test target via a `test { _ = @import("history_backend.zig"); }` block in main.zig.
- **Pty abstraction** in `apps/desktop/src/pty.zig` extended from 47-line stub to ~225 lines: real `Pty` struct with `VTable`, `Backend` enum (`conpty_windows / native_posix / script_posix / direct_posix / unavailable`), `currentBackend()` resolver, `SpawnOptions`, and four backend-specific spawn stubs returning `error.NotImplemented` with TODOs pointing at the real implementations in `windows_app.zig` / `posix_app.zig` / `posix_pty.zig`. Three new tests. All existing public symbols (`posix`, `isPosixPtyTarget`, `preferredBackend`, `backendName`) preserved.
- **Wave 3 native window scaffold** in `apps/desktop/src/posix_app.zig`: new `pub fn runNativeWindow(init_data) !void` returns `error.NotImplemented` with a comment block listing the Linux (X11 / Wayland) and macOS (Cocoa) implementation paths. Wired as opt-in via `ZIGGYZAG_NATIVE_WINDOW` env var at the top of `run()` — when the var is set, native window is tried first and we fall through to the existing terminal-attached launcher on failure. Default behaviour is unchanged.
...
- **Stale-shell-path resolver bug** (`apps/desktop/src/windows_app.zig`). `resolveShellPath` and `resolveAgentPath` returned configured paths (`profile.shell` in `desktop.conf`, `ZIGGYZAG_SHELL_PATH`, `ZIGGYZAG_AGENTD_PATH`) without checking they actually existed, so a stale config produced startup `CreateProcessFailed` even when a working `ziggyzag.exe` was sitting right next to the desktop binary. Both resolvers now route configured paths through a new `existingDup` helper that falls through to the next channel when the file is missing. Companion `scripts/doctor-desktop.ps1` diagnoses and repairs offending `desktop.conf` files in-place with a timestamped backup. See [`docs/reviews/2026-05-17-windows-debug.md`](docs/reviews/2026-05-17-windows-debug.md).
- **CreateProcessW rejected the env block with ERROR_INVALID_PARAMETER** (`apps/desktop/src/windows_app.zig`). `child_env.createWindowsBlock(...)` builds a UTF-16 environment block, but the flags passed to `CreateProcessW` in `startPtyForPane` did not include `CREATE_UNICODE_ENVIRONMENT` (0x00000400). Without it, Windows tried to parse the block as ANSI, saw every other byte as NUL, and returned `GetLastError = 87`. Flag added; constant carries a comment block pointing at the diagnosis. This was a root cause behind fresh-install `CreateProcessFailed` reports; the resolver fix above was a real but separate bug.
- **Dead ConPTY I/O bridge** (`apps/desktop/src/windows_app.zig`). `CREATE_NO_WINDOW` in `dwCreationFlags` is incompatible with `PROC_THREAD_ATTRIBUTE_PSEUDOCONSOLE`: the shell child attached to its own private conhost instead of the pseudoconsole, so no bytes ever crossed the pipes. `CreateProcessW` still returned success, which is why this stayed silent. A second defect — `UpdateProcThreadAttribute` passed `&hpc` instead of the `hpc` value — caused a stalled handshake, isolation-tested as a separate milder failure. Both fixed; flags are now `EXTENDED_STARTUPINFO_PRESENT | CREATE_UNICODE_ENVIRONMENT`. See [`docs/reviews/2026-05-17-windows-debug.md`](docs/reviews/2026-05-17-windows-debug.md).
- **Windows shell stayed in cooked line-input mode** (`apps/shell/src/main.zig`). `TerminalMode.enable()` was a no-op stub on Windows, so the shell never left ConPTY cooked line-input mode. The shell's `readLine` is a raw-mode reader; in cooked mode the console buffered the line and the reader never saw a submitted command. Implemented the real Windows branch: clears `ENABLE_PROCESSED_INPUT|ENABLE_LINE_INPUT|ENABLE_ECHO_INPUT`, sets `ENABLE_VIRTUAL_TERMINAL_INPUT`, degrades to `null` if stdin is not a console; `restore()` puts the saved mode back. Verified by per-keystroke manual-echo redraws + post-Enter command-output burst + integration `status: ok`. Together with the bridge fix this is what makes the Windows desktop host actually run typed commands.
- **Desktop linked as console subsystem** (`build.zig`). `ziggyzag-desktop.exe` was PE subsystem 3 (console) and spawned a stray `conhost.exe`. Fixed: `desktop_exe.subsystem = .windows`, matching the launcher. Both exes are now GUI subsystem.
- **`readLine` rejected a bare CR** (`apps/shell/src/main.zig`). `readLine` submitted only on `\n` and silently discarded `\r`; any client delivering a bare CR got a dead Enter. Now submits on `\r` as well as `\n`, with a `pending_cr_swallow` flag that coalesces a following `\n` so `\r\n` counts as one submit. The Windows desktop path (LF only) is unaffected.
