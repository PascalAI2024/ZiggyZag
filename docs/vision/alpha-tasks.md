...
- [ ] Make the settings overlay write a valid `desktop.conf`, validate changes before saving, expose profile fields, and recover cleanly from config errors. Progress: `config.zig` now has round-tripping `serialize`, a `validate` that rejects every invalid field, and profile-field parsing/serialization with tests. The remaining work is wiring the Windows settings overlay UI (`windows_app.zig`) to call serialize/validate on save.
