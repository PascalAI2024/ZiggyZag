...
| macOS/Linux desktop host | Alpha | `ziggyzag-desktop` builds and launches ZiggyZag in the calling terminal (POSIX PTY relay → `script(1)` → stdio). On macOS, `ZIGGYZAG_NATIVE_WINDOW=1` opens a native Cocoa NSWindow/NSView terminal: CoreText/CoreGraphics grid renderer, dynamic resize (windowDidResize:), mouse-wheel scrollback, command palette (`Ctrl+Shift+P`), scrollback search (`Ctrl+Shift+F`), quick select (`Ctrl+Shift+O`), settings overlay (`Ctrl+,`), theme cycling with live OSC 7777 broadcast (`Ctrl+Shift+T`), copy visible text (`Cmd+Shift+C`) via NSPasteboard, Cmd+V paste (bracketed-paste aware), application-cursor arrows, and AgentD universal input (`Ctrl+Space` — type natural language, preview suggested command, insert into prompt). AgentD is pre-forked before NSApp.run. Text rendering uses standard CG coordinates (y=0 at bottom). Linux native window remains planned. |
...
- [ ] Harden AgentD panel workflows: read-only tool browsing from the UI, build-action approval, audit export, provider streaming, tool cancellation/timeouts, clearer error states, and stronger secret redaction. Progress: the agentd sidecar now enforces a per-tool execution timeout with a watchdog kill, returns consistent snake_case structured error envelopes on every failure path, exposes a stateless `audit/policy` export method, and has substantially stronger secret redaction (Authorization headers, Slack/GitHub/AWS tokens, PEM private-key blocks, secret-assignment patterns). `windows_app.zig` now shows an inline "No AI provider reachable" hint with setup instructions when no provider is detected. `macos_app.zig` now has: (1) protocol bug fixed — requests use flat wire format matching agentd's parser; (2) terminal.write and zig.build require explicit Y/N approval before anything reaches the PTY; (3) tools/list responses parsed and displayed with Up/Down navigation and [!] approval markers; (4) error envelopes surfaced in the overlay with red highlighting; (5) Esc dismisses sub-modes in order (approval → tools browse → input → close). Remaining: provider streaming, tool cancellation, audit export, parity of all features between macOS and Windows panels.
