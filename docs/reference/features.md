...
| Desktop terminal | Native hosts for Windows (Win32 + ConPTY) and macOS (Cocoa + CoreText). PTY-backed split panes (Windows), styled grid, bounded scrollback, alternate screen, 16/256/RGB colors, bracketed paste, app cursor mode, mouse-wheel reports, command palette, scrollback search, quick select, settings/themes overlay, theme cycling with live OSC 7777 broadcast, copy visible text, Cmd+V paste, mouse selection with double/triple-click word/line select and Cmd+C copy, semantic prompt zones, prompt-jump navigation (`Cmd+Up`/`Cmd+Down`), SGR render attributes (underline/italic/strikethrough/bold/double-underline) + 256-color and RGB truecolor, AgentD universal input (type → preview → insert), shell integration events, and `zig build bundle` for a `.app` bundle. Tabs and deeper session restore remain planned hardening work. |
| Unicode terminal | Cells store Unicode scalars, decode UTF-8 safely with invalid-byte replacement, emit UTF-8 text for copy/search/extraction, track wide-cell continuations for CJK and emoji, and preserve zero-width combining marks in the correct cell. Emoji grapheme clusters, fallback fonts, ligatures, and broader TUI compatibility remain planned hardening work. |
| Agent runtime | `ziggyzag-agentd` provides a slim Zig-native JSON-lines agent process with tool discovery, schemas, approval metadata, cancel protocol, provider streaming, redacted bounded local file/search/git output, host-action audit events, terminal host actions, and Ollama/OpenAI-compatible provider calls. |
| VT conformance | Purpose-built conformance harness binary (`zig build vt-conformance`) with grid snapshot diff. 15/15 test cases pass covering CSI/SGR parsing, cursor state, scroll regions, alternate screen, and malformed escape recovery. Runs in CI on every push. |
| Tests | ~220 Zig unit tests across 4 binaries, Windows smoke script, POSIX smoke script, headless macOS/Linux desktop smoke test (`zig build smoke-desktop`), and GitHub Actions CI on Windows, Linux, and macOS. |
